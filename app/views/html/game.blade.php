    ...
    <h2 class="text-center" style="font-size:20px; margin-top:2px;"><button type="button" class="btn btn-default pull-left btn-flat" style="margin-left:0px; margin-top:-6px; outline:none;" onclick="closeDetails('{{ $match->id }}');"><i class="fa  fa-angle-double-up"></i> Hide game{{ (count($match->getGames()) > 1 ? 's' : '') }}</button>
        ...
        @elseif(count($match->getGames()) == 1 && $match->maxGames == 1)
            ...
            @if($match->getGames()[0]->vodType != null)
               <span class='label label-danger' style='font-size:16px;'><span data-container='body' onclick="getGameVod('{{ $match->getGames()[0]->gameId }}');" style="cursor: pointer;" data-toggle="tooltip" data-placement="right" title="Watch Game"><i id='game-{{ $game->gameId }}-play' class="fa fa-youtube-play"></i></span></span>
            @endif
        @endif
